...
Working prototype. In-memory only (no persistence). Single-node.
...
- **Axum** HTTP framework
- **Shared state**: `Arc<Mutex<QueryStore>>` — simple, correct. Swap for `RwLock` if
  read-heavy profiling shows contention.
- **No persistence**: Queries live in memory only. Will add SQLite or Cloud SQL later.
